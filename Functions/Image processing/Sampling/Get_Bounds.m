function imdat = Get_Bounds(varargin)
%either specify the x and y coordinates of the curve, or specify pass the
%imdat data struct generated by the imagetracker function

%function example:
%splinedat{imno} = Get_Bounds(imdat{imno},offset);
%offsets = Get_Bounds(X0,Y0,3);

%Output will be a struct in the form:
%Output.c0 : the origin spline
%Output.c1 : the negative spline projected by -offset
%Output.c2 : the positive spline projected by +offset

%The algorithm accepts arrays of objects, e.g. mulitple legs, in which case
...
if nargin == 2
    imdat = varargin{1};
    offset = varargin{2};
    nLegs = size(imdat.fitting.legobj,2);
    for n = 1:nLegs
        spline = imdat.fitting.legobj{n}.spline;
        pts = fnplt(spline);
        
        %original splines
        imdat.sampling.c0{n} = [pts(1,:)',pts(2,:)'];
        
        %offset 1
        [x1,y1] = offsetCurve(pts(1,:),pts(2,:),-offset,gca);
        imdat.sampling.c1{n} = [x1',y1'];

        %offset 2
        [x2,y2] = offsetCurve(pts(1,:),pts(2,:),+offset,gca);
        imdat.sampling.c2{n} = [x2',y2'];
    end
...
elseif nargin == 3
    ...
    for n = 1:nLegs
        %original pts
        imdat.sampling.c0{n}=[X0{n}',Y0{n}'];
        
         %offset 1
        [x1,y1] = offsetCurve(X0{n},Y0{n},offset,gca);
        imdat.sampling.c1{n} = [x1',y1'];
        
        %offset 2
        [x2,y2] = offsetCurve(X0{n},Y0{n},-offset,gca);
        imdat.sampling.c2{n} = [x1',y1'];
    end
end
imdat.sampling.nLegs = nLegs;
end
